...
const AdminDashboard = () => {
  ...
  const fetchGuests = useCallback(() => {
    ...
    axios
      ...
      .then((res) => {
        // console.log("Raw data from API:", JSON.stringify(res.data, null, 2)); // Untuk debugging data mentah
        setGuests(res.data);
        setIsLoading(false);
      })
      .catch((err) => {
  ...
  }, []); // API_BASE_URL bisa ditambahkan jika dinamis, tapi karena konstan, tidak wajib
  ...
  const formatDate = (dateString) => {
    ...
    let date;
    const initialDate = new Date(dateString); // Coba parsing langsung

    if (!isNaN(initialDate.getTime())) {
      date = initialDate;
    } else {
      // Fallback parsing untuk format 'YYYY-MM-DD HH:MM:SS' atau 'YYYY-MM-DDTHH:MM:SS'
      const parts = dateString.replace("T", " ").split(/[- :]/);
      if (parts.length >= 3) {
        const year = parseInt(parts[0], 10);
        const month = parseInt(parts[1], 10) - 1; // Bulan di JS 0-indexed
  ...
  const getFilteredGuests = () => {
    let filtered = guests.filter((guest) => {
      ...
      const searchMatch =
        ...
        (guest.topik_konsultasi &&
          guest.topik_konsultasi.toLowerCase().includes(searchTermLower)); // Tambahkan pencarian di topik_konsultasi
    ...
    return filtered.sort((a, b) => {
      switch (sortBy) {
        case "newest":
          // Handle jika tanggal_kehadiran null atau undefined
          ...
          if (dateB) return 1; // b lebih baru
          if (dateA) return -1; // a lebih baru
          return 0;
        case "oldest":
        ...
        case "status":
          const statusOrder = {
            "Belum Diproses": 0,
            null: 0, // Anggap null sama dengan Belum Diproses
            Diproses: 1,
            Selesai: 2,
          };
          // Gunakan a.status dan b.status
          ...
          ); // Anggap status tidak dikenal paling akhir
        default:
          return 0;
      }
    });
    ...
    (guestId, newStatus) => {
      ...
      axios
        .put(`${API_BASE_URL}/tamu/${guestId}`, {
          ...guestToUpdate, // Kirim seluruh objek tamu yang diupdate
          status: newStatus,
        })
        .then(() => {
          ...
          // fetchGuests(); // Opsional: panggil fetchGuests lagi jika ingin data terbaru dari server setelah update
        })
        .catch((err) => {
          console.error("Error updating guest:", err);
          setError("Gagal mengupdate status tamu.");
          setGuests(previousGuests); // Rollback jika gagal
        });
    },
    [guests, selectedGuest, API_BASE_URL] // tambahkan API_BASE_URL
  );
  ...
  const getPriorityIndicator = (guest) => {
    ...
    const today = new Date().toISOString().split("T")[0];
    const isToday = guest.tanggal_kehadiran.startsWith(today); // Periksa apakah tanggal dimulai dengan tanggal hari ini
    const isUrgent =
      guest.keperluan &&
  ...
  const truncateText = (text, maxLength = 30) => {
    if (text === null || typeof text === "undefined") return ""; // Handle null atau undefined
    const stringText = String(text); // Pastikan itu string
    ...
    >
            ...
            {isLoading ? (
              ...
              <>
                  ...
                  <table className="data-table">
                    ...
                    <tbody>
                      {displayedGuests.map((guest, index) => (
                        <tr
                          key={guest.id} // Pastikan guest.id unik
                        ...
                        >
                          ...
                          {/* --- KOLOM KEPERLUAN YANG DIMODIFIKASI --- */}
                ...
                {getFilteredGuests().length > 10 &&
                  !showAll && ( // Tampilkan hanya jika lebih dari 10 dan belum showAll
                    <div className="pagination-footer">
                      ...
                      <button
                        onClick={() => setShowAll(true)} // Ubah menjadi true saat diklik
                ...
                {showAll &&
                  getFilteredGuests().length > 10 && ( // Tombol untuk "Tampilkan Lebih Sedikit"
                    <div className="pagination-footer">
                      <p className="results-count-info">
      ...
      {selectedGuest && (
        <GuestDetailModal
          ...
          formatDate={formatDate} // Kirim formatDate ke modal jika perlu
        />
      )}
    </div>
  );
