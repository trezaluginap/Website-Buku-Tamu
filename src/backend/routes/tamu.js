...
router.put("/:id", (req, res) => {
  ...
  allowedFieldsToUpdate.forEach((field) => {
    if (receivedFields.hasOwnProperty(field)) {
      fieldsToSet.push(`\`${field}\` = ?`); // Menggunakan backtick untuk nama kolom
  ...
  //     // Cari index yang benar untuk 'values' berdasarkan 'fieldsToSet'
  //     let valueIndex = -1;
  //     for(let i=0; i<fieldsToSet.length; i++) {
  //         if (fieldsToSet[i] === `\`dokumentasi\` = ?`) {
  //             valueIndex = i;
  //             break;
  //         }
  //     }
